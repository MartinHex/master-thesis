    ...
    def train(self,epochs=1, device = None):
        ...
        for epoch in range(epochs):
            ...
            for k in range(t - 1):
                ...
                if k == t - 2:
                    denominator = 1 + gamma_k * v[k].(u_key)
                    sum_factors.append(gamma_k/denominator)))
                factor = sum_factors[k]
                sum = sum.add(v[k].mul(sum_factors))
            v.append(u_t.sub(sum)) #Equation 28
            current_average = sample_weight.add(current_average, alpha = l).div(l + 1)
            gamma_t = (self.beta * (t - 1)) / t #Equation 17.
            nominator = gamma_t * (t * u_t.matmul(current_delta) - u_key.matmul(v[t]))
        ...
        new_weights =  initial_weights.sub(current_delta.cpu() / self.shrinkage) #Gradient from Equation 2
