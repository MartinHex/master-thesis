...
class FedPaClient(Base_Client):
    ...
    def train(self,epochs=1, device = None):
        ...
        # https://github.com/alshedivat/fedpa/blob/master/federated/inference/local.py
        x_0 = self._model_weight_to_array(self.model.get_weights())
        num_samples = epochs
        samples = []
        for epoch in range(epochs):
            samples.append(self._model_weight_to_array(self._produce_iasg_sample(device)))
        with torch.no_grad():
            rho = self.shrinkage
            dp = defaultdict(list)
            samples_ra = samples[0]
            delta = x_0.to(device) - samples_ra
            ...
            new_weights = x_0.sub(final_delta.cpu())
            self.set_weights(self._array_to_model_weight(new_weights))
