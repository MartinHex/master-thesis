...
bandwidth_methods = ['plugin','crossval','local','scott','silverman']
batch_size = 16
...
for bandwidth_selection in bandwidth_methods:
    ...
    for i in range(cv):
        ...
        init_Model = Model()
        clients = [Client(Model(),dl) for dl in train_dls]
        # train Clients
    ...
    with open(os.path.join(out_path,bandwidth_selection+'.json'),'w') as f:
        json.dump(res,f)
...
# ax.set_xscale('log')
# ax.legend()
# plt.savefig(os.path.join(out_path,'Server_loss.png'))
# plt.show()
#
# # Plot losses
...
# ax.set_xscale('log')
# ax.legend()
# plt.savefig(os.path.join(out_path,'Server_accuracy.png'))
# plt.show()
